@extends('admin.main.admin')

@section('content')

<div class="secClara pt-4">
   

   <div class="seccionTipo2 pt-4">
        <h2>EVENTOS</h2>
       
        @if($item->idEvento==0)
        <form method="post" action="{{route('creaEvento')}}" id="f1" onsubmit="showOverlay()" enctype="multipart/form-data">
        @else
        <form method="post" action="{{route('upEvento')}}" id="f1" onsubmit="showOverlay()" enctype="multipart/form-data">
        @endif
         @csrf

            @if($item->idEvento>0)
                <div class="mb-3 d-none"><input type="text" class="form-control" name="idEvento" value="{{$item->idEvento}}"></div>
            @endif
           
            <div class="mb-3">
                <label for="cEvento" class="form-label">Nombre de la evento</label>
                <input type="text" class="form-control" name="nombre" value="@if($item->idEvento>0) {{$item->cEvento}} @endif" required>
            </div>
            <div class="mb-3">
                <label for="cLugar" class="form-label">Lugar de la actividad</label>
                <input type="text" class="form-control" name="lugar" value="@if($item->idEvento>0) {{$item->cLugar}} @endif"] required>
            </div>
            <div class="mb-3">
                <label for="cDesc" class="form-label">Descripción</label>
                <textarea class="form-control" name="desc" rows="3" required>@if($item->idEvento>0) {{$item->cDescripcion}} @endif</textarea>
            </div>
            <div class="mb-3">
                <label for="cDesc" class="form-label">Fecha</label> 
                @if($item->idEvento>0)
                <input type="date" name="fecha" step="1" value="{{$item->dEvento}}" required>
                @else
                <input type="date" name="fecha" step="1" value="{{$Hoy()}}" required>
                @endif
            </div>
            <div class="row">
            <div class="mb-3 col-4">
         
                    @if($item->idEvento>0)
                        <img src="{{asset('images/content/eventos').'/'.$item->idEvento.'.webp'}}" class="d-block w-100" alt="...">
                    @else
                         <img src="{{asset('images/si.png')}}" class="d-block w-100" alt="...">
                    @endif
             
                
            </div>
            <div class="mb-3 col-8">
                <label for="formFile" class="form-label">Suba una imagen</label>
                @if($item->idEvento==0)
                     <input class="form-control" type="file" name="archivo" required>
                @else
                     <input class="form-control" type="file" name="archivo">
                @endif
               
            </div>
            </div>

            <div class="mb-3">
                <label for="cLink" class="form-label">Link relacionado</label>
                <input type="text" class="form-control" name="link" value="@if($item->idEvento>0) {{$item->cLink}} @endif" required>
            </div>

            <button id="btnEditar" type="submit" class="btn btn-primary">
        
            @if($item->idEvento > 0)
                Actualizar
            @else
                Registrar
            @endif
        
            </button>

         
            
        
            </form>
        </div>


          


</div>


@endsection